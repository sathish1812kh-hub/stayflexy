...
const DEFAULT_RESERVATIONS: Reservation[] = [
  {
    ...
    amount: 450.0,
    notes:
      'EMAIL:alice.vance@example.com | PHONE:+1 555-0199 | NATIONALITY:United States | ID_TYPE:PASSPORT | ID_NUMBER:P829103 | DOB:1988-04-12 | RATE:150.00 | DISCOUNT:0.00 | TAX:54.00 | Requires late checkout at 1:00 PM.',
  ...
  },
  {
    ...
    amount: 770.0,
    notes:
      'EMAIL:robert.d@example.com | PHONE:+91 9988223311 | NATIONALITY:India | ID_TYPE:NATIONAL_ID | ID_NUMBER:IN-A8291 | DOB:1991-08-25 | RATE:110.00 | DISCOUNT:0.00 | TAX:92.40 | Corporate account booking from Socifyy.',
  ...
  },
  {
    ...
    amount: 450.0,
    notes:
      'EMAIL:clara@tardis.org | PHONE:+44 7911 123456 | NATIONALITY:United Kingdom | ID_TYPE:DRIVERS_LICENSE | ID_NUMBER:DL-UK829 | DOB:1993-02-14 | RATE:150.00 | DISCOUNT:0.00 | TAX:54.00 | Prefers high floor room. Poolside delivery requested.',
  ...
  },
  {
    ...
    amount: 600.0,
    notes:
      'EMAIL:danny.pink@school.edu | PHONE:+44 7911 987654 | NATIONALITY:United Kingdom | ID_TYPE:PASSPORT | ID_NUMBER:P001928 | DOB:1989-11-05 | RATE:150.00 | DISCOUNT:0.00 | TAX:72.00 | Anniversary celebration. Complementary setup.',
  ...
  },
  {
    ...
    amount: 1176.0,
    notes:
      'EMAIL:guest.101@example.com | PHONE:+1 555-0101 | NATIONALITY:United States | ID_TYPE:PASSPORT | ID_NUMBER:P-ID-101 | DOB:1990-01-01 | RATE:150.00 | DISCOUNT:0.00 | TAX:126.00',
  ...
  },
  {
    ...
    amount: 1176.0,
    notes:
      'EMAIL:guest.102@example.com | PHONE:+1 555-0102 | NATIONALITY:United States | ID_TYPE:PASSPORT | ID_NUMBER:P-ID-102 | DOB:1990-01-01 | RATE:150.00 | DISCOUNT:0.00 | TAX:126.00',
  ...
  },
  {
    ...
    amount: 1176.0,
    notes:
      'EMAIL:guest.103@example.com | PHONE:+1 555-0103 | NATIONALITY:United States | ID_TYPE:PASSPORT | ID_NUMBER:P-ID-103 | DOB:1990-01-01 | RATE:150.00 | DISCOUNT:0.00 | TAX:126.00',
  ...
  },
  {
    ...
    amount: 1176.0,
    notes:
      'EMAIL:guest.201@example.com | PHONE:+1 555-0201 | NATIONALITY:United States | ID_TYPE:PASSPORT | ID_NUMBER:P-ID-201 | DOB:1990-01-01 | RATE:150.00 | DISCOUNT:0.00 | TAX:126.00',
  ...
  },
  {
    ...
    amount: 1176.0,
    notes:
      'EMAIL:guest.202@example.com | PHONE:+1 555-0202 | NATIONALITY:United States | ID_TYPE:PASSPORT | ID_NUMBER:P-ID-202 | DOB:1990-01-01 | RATE:150.00 | DISCOUNT:0.00 | TAX:126.00',
  ...
  },
  {
    ...
    amount: 1176.0,
    notes:
      'EMAIL:guest.301@example.com | PHONE:+1 555-0301 | NATIONALITY:United States | ID_TYPE:PASSPORT | ID_NUMBER:P-ID-301 | DOB:1990-01-01 | RATE:150.00 | DISCOUNT:0.00 | TAX:126.00',
  ...
  },
  {
    ...
    amount: 1176.0,
    notes:
      'EMAIL:guest.901@example.com | PHONE:+1 555-0901 | NATIONALITY:United States | ID_TYPE:PASSPORT | ID_NUMBER:P-ID-901 | DOB:1990-01-01 | RATE:150.00 | DISCOUNT:0.00 | TAX:126.00',
  ...
  },
  {
    ...
    amount: 1176.0,
    notes:
      'EMAIL:guest.902@example.com | PHONE:+1 555-0902 | NATIONALITY:United States | ID_TYPE:PASSPORT | ID_NUMBER:P-ID-902 | DOB:1990-01-01 | RATE:150.00 | DISCOUNT:0.00 | TAX:126.00',
...
export default function BookingsGanttPage() {
  ...
  useEffect(() => {
    setIsMounted(true)
    // Anchor the room-grid timeline to the REAL current date on mount.
    // Initial state uses a fixed date so SSR/hydration stays deterministic;
    // the actual system date is applied here (client-side) to avoid a
    // server/client hydration mismatch, and keeps the "now" indicator on-grid.
  ...
  const [advancePaymentMethod, setAdvancePaymentMethod] = useState<'CASH' | 'CARD' | 'ONLINE'>(
    'CASH',
  )
  ...
  async function loadData() {
    const activeHotelId =
      typeof window !== 'undefined'
        ? localStorage.getItem('sf_selected_hotel') || 'h1-resort-goa'
        : 'h1-resort-goa'
    ...
    if (remoteBookings && remoteBookings.length > 0) {
            ...
            b.status === 'PENDING' || b.status === 'CONFIRMED'
              ...
              : b.status === 'CHECKED_IN'
                ...
                : b.status === 'CHECKED_OUT'
                  ...
                  : 'CANCELLED',
          amount:
            b.rooms?.reduce((acc: number, r: any) => acc + (r.totalRoomAmount || 0), 0) || 120.0,
      ...
      const savedRes =
        typeof window !== 'undefined' ? localStorage.getItem('sf_reservations') : null
      if (savedRes) {
        try {
          const list = JSON.parse(savedRes) as Reservation[]
          const filtered = list.filter(
            (r) => !(!r.id?.endsWith('-default') && r.checkIn?.startsWith('2026-06')),
          )
  ...
  useEffect(() => {
    ...
    loadData()
    // Poll the backend so rooms checked in / booked in ANOTHER browser window
    // show up here within a few seconds without a manual reload. Visible-tab only.
  ...
  const handleAddIncidental = (resId: string) => {
    ...
    const updated = reservations.map((r) => {
      if (r.id === resId) {
        ...
        const newCharges = [
          ...charges,
          { name: incidentalName, amount: amt, timestamp: new Date().toISOString() },
        ]
  ...
  const handlePostPayment = (resId: string) => {
    ...
    const updated = reservations.map((r) => {
      if (r.id === resId) {
        ...
        const newPayments = [
          ...payments,
          { method: paymentMethod, amount: amt, timestamp: new Date().toISOString() },
        ]
  ...
  const handleSwapReservations = async (resIdA: string, resIdB: string) => {
    ...
    if (successA && successB) {
      ...
      setTimeout(() => {
        alert(
          `Successfully swapped rooms! ${resA.guestName} is now in Room ${roomB_number}, and ${resB.guestName} is in Room ${roomA_number}.`,
        )
      }, 300)
    }
  }

  const handleGridDrop = async (
    bookingId: string,
    targetRoomId: string,
    ...
    const nights = Math.max(
      1,
      Math.round((endOrig.getTime() - startOrig.getTime()) / (1000 * 60 * 60 * 24)),
    )
    ...
    if (hasConflict) {
      ...
      const conflictingRes = reservations.find((r) => {
        if (r.id === bookingId || r.roomId !== targetRoomId || r.status === 'CANCELLED')
          return false
        const r1 = new Date(r.checkIn)
        const r2 = new Date(r.checkOut)
        const n1 = new Date(newCheckIn)
  ...
  const handleCreateBooking = async (e: React.FormEvent) => {
    ...
    if (roomId && roomId !== 'UNASSIGNED') {
      ...
      if (hasConflict) {
        alert(
          'Validation Failed: The selected room is already booked for one or more of the requested dates.',
        )
        return
      }
    }
      ...
      if (typeof window !== 'undefined') {
        ...
        if (saved) {
          ...
          const matchIndex = list.findIndex(
            (r) => r.id === newRes.id || r.guestName === combinedGuestName,
          )
    ...
    >
      <div style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <div>
            ...
            <p style={{ fontSize: '13px', color: 'var(--text-muted)' }}>
              Visualize occupancy overlaps and process arrival schedules
            </p>
          </div>
        ...
        >
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr 1fr 1fr', gap: '12px' }}>
            <div style={{ display: 'flex', flexDirection: 'column', gap: '4px' }}>
              <label style={{ fontSize: '10px', fontWeight: 600, color: 'var(--text-muted)' }}>
                Search Bookings
              </label>
              <input
                type="text"
            ...
            >
              <button
                style={{
                  padding: '6px 14px',
                  borderRadius: '6px',
                  background: viewScale === 'DAY' ? 'var(--primary)' : 'transparent',
                  color: viewScale === 'DAY' ? '#060913' : '#cbd5e1',
                  border: 'none',
                  fontWeight: 600,
                  fontSize: '11px',
                  cursor: 'pointer',
              ...
              <button
                style={{
                  padding: '6px 14px',
                  borderRadius: '6px',
                  background: viewScale === 'WEEK' ? 'var(--primary)' : 'transparent',
                  color: viewScale === 'WEEK' ? '#060913' : '#cbd5e1',
                  border: 'none',
                  fontWeight: 600,
                  fontSize: '11px',
                  cursor: 'pointer',
              ...
              <button
                style={{
                  padding: '6px 14px',
                  borderRadius: '6px',
                  background: viewScale === 'TWO_WEEKS' ? 'var(--primary)' : 'transparent',
                  color: viewScale === 'TWO_WEEKS' ? '#060913' : '#cbd5e1',
                  border: 'none',
                  fontWeight: 600,
                  fontSize: '11px',
                  cursor: 'pointer',
              ...
              <button
                style={{
                  padding: '6px 14px',
                  borderRadius: '6px',
                  background: viewScale === 'MONTH' ? 'var(--primary)' : 'transparent',
                  color: viewScale === 'MONTH' ? '#060913' : '#cbd5e1',
                  border: 'none',
                  fontWeight: 600,
                  fontSize: '11px',
                  cursor: 'pointer',
          ...
          >
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                ...
                <h3
                  style={{ fontSize: '16px', fontWeight: 700, color: 'var(--primary)', margin: 0 }}
                >
                  Advanced Booking Creation Wizard
                </h3>
              </div>
              <button
                type="button"
            ...
            >
              ...
              <div>
                  ...
                  >
                    ...
                    >
                      ...
                      <span>
                        ...
                        {Math.max(
                          0,
                          finalLedgerTotal - (parseFloat(advancePaymentAmount) || 0),
                        ).toFixed(2)}
                      </span>
                    </div>
                  </div>
                ...
                >
                  <label style={{ fontSize: '10px', fontWeight: 600, color: 'var(--text-muted)' }}>
                    Additional Special Requests & Notes
                  </label>
                  <textarea
                    value={notes}
              ...
              <div>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '14px' }}>
                  ...
                  >
                    ...
                    <input
                      type="text"
                      value={creationTransactionId}
                      onChange={(e) => setCreationTransactionId(e.target.value)}
                      style={{
                        padding: '8px 12px',
                        background: 'rgba(255,255,255,0.03)',
                        border: '1px solid var(--border-card)',
        ...
        <div
          style={{
            display: 'grid',
            gridTemplateColumns: showUnassignedTray ? '1fr 280px' : '1fr',
            gap: '20px',
            alignItems: 'start',
          }}
        >
            ...
            <div style={{ minWidth: '1000px', position: 'relative' }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                {filteredRooms.map((room, roomIdx) => {
                  ...
                  const roomRes = filteredReservations.filter(
                      ...
                      if (viewScale === 'DAY') {
                        ...
                        if (checkOutDate > focusStart && checkInDate < focusEnd) {
                          ...
                          const startFraction = Math.max(
                            0,
                            (checkInDate.getTime() - focusStart.getTime()) / (1000 * 60 * 60),
                          )
                          const endFraction = Math.min(
                            24,
                            (checkOutDate.getTime() - focusStart.getTime()) / (1000 * 60 * 60),
                          )
                        ...
                        const focusEnd = parseSafeDateTime(
                        ...
                        if (d2 > focusStart && d1 < focusEnd) {
                          ...
                          const startCol =
                            d1 < focusStart
                              ? 0
                              : Math.floor(
                                  (d1.getTime() - focusStart.getTime()) / (1000 * 60 * 60 * 24),
                                )
                          const endCol =
                            d2 > focusEnd
                              ? timelineDays.length
                              : Math.ceil(
                                  (d2.getTime() - focusStart.getTime()) / (1000 * 60 * 60 * 24),
                                )
                    ...
                    >
                      ...
                      <div
                        style={{
                          ...
                          borderRadius: '4px',
                          display: 'grid',
                          gridTemplateColumns: `repeat(${timelineDays.length}, 1fr)`,
                        }}
                      >
                        ...
                        {timelineDays.map((day, idx) => (
                          <div
                            ...
                            onDoubleClick={() =>
                              handleCellDoubleClick(room.id, room.roomNumber, day)
                            }
                            onDragOver={(e) => {
                              e.preventDefault()
                            ...
                            title={
                              allocatingResId
                                ? `Allocate guest here: Room ${room.roomNumber} - ${getDayLabel(day)}`
                                : 'Double-click to create reservation for this room & date'
                            }
                          />
                        ))}
                        ...
                        {sortedMapped.map((item) => {
                          ...
                          const parsedRate =
                            res.baseRate || parseFloat(parseMetaField(res.notes, 'RATE') || '150')
                          ...
                          return (
                            ...
                            >
                              ...
                              <span className={`res-tooltip-text${tooltipAlignClass}`}>
                                ...
                                <br />
                                <span style={{ color: 'var(--text-muted)' }}>Room:</span> Room{' '}
                                {res.roomNumber} ({isHourlyRes ? 'Hourly' : 'Daily'})
                                <br />
                                <span style={{ color: 'var(--text-muted)' }}>Folio Amt:</span> $
                                {res.amount?.toFixed(2)} (${parsedRate}/base)
                                {res.notes && (
                                  <>
                                    <br />
                                    <span
          ...
          {showUnassignedTray && (
            ...
            >
              ...
              >
                ...
                >
                  {
                    filteredReservations.filter((res) => !res.roomId && res.status !== 'CANCELLED')
                      .length
                  }{' '}
                  Stays
                </span>
              ...
              {allocatingResId && (
                ...
                >
                  <strong>Allocation Mode Active:</strong> Click any empty date cell on the room
                  grid to allocate this booking.
                  <button
              ...
              >
                {filteredReservations
                  .filter((res) => !res.roomId && res.status !== 'CANCELLED')
                  .map((res) => {
                    const isSelected = allocatingResId === res.id
                    const matchedRt = roomTypes.find(
                      (rt) =>
                        rt.id === res.roomId || rt.id === parseMetaField(res.notes, 'CATEGORY_ID'),
                    )
                    return (
                      <div
                        key={res.id}
                      ...
                      >
                        ...
                        <div
                          style={{ fontSize: '10px', color: 'var(--text-muted)', marginTop: '6px' }}
                        >
            ...
            >
              ...
              {(() => {
                ...
                const nationalityVal =
                  selectedRes.nationality ||
                  parseMetaField(selectedRes.notes, 'NATIONALITY') ||
                  'United States'
                const idTypeVal =
                  selectedRes.idType || parseMetaField(selectedRes.notes, 'ID_TYPE') || 'PASSPORT'
                const idNumberVal =
                  selectedRes.idNumber ||
                  parseMetaField(selectedRes.notes, 'ID_NUMBER') ||
                  'CONTACTLESS_OCR'
                const dobVal =
                  selectedRes.dob || parseMetaField(selectedRes.notes, 'DOB') || '1990-01-01'

                const rateValSelected =
                  selectedRes.baseRate ||
                  parseFloat(parseMetaField(selectedRes.notes, 'RATE') || '150')
                const discountValSelected =
                  selectedRes.discount ||
                  parseFloat(parseMetaField(selectedRes.notes, 'DISCOUNT') || '0')
                const taxValSelected =
                  selectedRes.tax || parseFloat(parseMetaField(selectedRes.notes, 'TAX') || '18')
                const finalValSelected =
                  selectedRes.amount ||
                  parseFloat(parseMetaField(selectedRes.notes, 'FINAL') || '168')

                const charges = selectedRes.charges || []
                const payments = selectedRes.payments || []
                const roomCharge = finalValSelected
                const totalCharges =
                  roomCharge + charges.reduce((acc: number, c: ChargeItem) => acc + c.amount, 0)
                const totalPayments = payments.reduce(
                  (acc: number, p: PaymentItem) => acc + p.amount,
                  0,
                )
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
                    ...
                    >
                      ...
                      >
                        <Globe size={12} />{' '}
                        <span>
                          {nationalityVal} ({idTypeVal}: {idNumberVal})
                        </span>
                      </div>
                      <div
                        style={{
                    ...
                    >
                      ...
                      >
                        Digital Signature (MAGICLINK REGISTRATION)
                      </span>
                      <div
                        style={{
                          fontFamily: 'cursive',
                    ...
                    >
                      ...
                      <div style={{ display: 'flex', flexDirection: 'column', gap: '4px' }}>
                        ...
                        >
                          Assigned Room (Quick Swap/Upgrade)
                        </label>
                        <select
                        ...
                        >
                          <option value={selectedRes.roomId}>
                            Room {selectedRes.roomNumber} (Current)
                          </option>
                          {rooms
                            .filter((r) => r.status === 'AVAILABLE' && r.id !== selectedRes.roomId)
                            .map((r) => (
                              <option key={r.id} value={r.id}>
                                Room {r.roomNumber} (
                                {roomTypes.find((rt) => rt.id === r.roomTypeId)?.name || 'Standard'}
                                )
                              </option>
                            ))}
                        </select>
                        ...
                        {!isEditingDates ? (
                          ...
                          >
                            <span style={{ fontSize: '12px', fontWeight: 600, color: '#fff' }}>
                              {selectedRes.checkIn} to {selectedRes.checkOut}
                            </span>
                            <button
                              onClick={() => {
                          ...
                          >
                            <input
                              type="date"
                              className="input-field"
                              style={{ padding: '3px 6px', fontSize: '11px', width: '110px' }}
                              value={editCheckIn}
                            ...
                            <div style={{ display: 'flex', gap: '4px', marginLeft: 'auto' }}>
                              <button
                                onClick={() => handleSaveEditedDates(selectedRes.id)}
                                style={{
                                  padding: '4px 8px',
                                  borderRadius: '4px',
                    ...
                    >
                      ...
                      >
                        ...
                        <span style={{ color: '#fff', fontWeight: 600 }}>
                          Stayflexi_{selectedRes.roomNumber}
                        </span>
                      </div>
                    </div>
                    ...
                    >
                          ...
                          style={{
                            ...
                            background:
                              balanceDue <= 0
                                ...
                                : 'rgba(239, 68, 68, 0.15)',
                            color: balanceDue <= 0 ? '#10b981' : '#ef4444',
                          }}
                        >
                          {balanceDue <= 0
                            ? 'FOLIO SETTLED'
                      ...
                      <div
                        style={{
                          ...
                          fontSize: '12px',
                          fontWeight: 700,
                          color: balanceDue <= 0.01 ? '#10b981' : '#ef4444',
                          borderTop: '1px solid rgba(255,255,255,0.05)',
                          paddingTop: '4px',
                        }}
                      >
                      ...
                      >
                        ...
                        <div style={{ display: 'flex', gap: '4px' }}>
                          <input
                            ...
                            value={incidentalName}
                            onChange={(e) => setIncidentalName(e.target.value)}
                          />
                          <input
                            type="number"
                            className="input-field"
                          ...
                          />
                          <button
                            onClick={() => handleAddIncidental(selectedRes.id)}
                            style={{
                              padding: '0 8px',
                              height: '24px',
                              borderRadius: '4px',
                        ...
                        <div style={{ display: 'flex', gap: '4px' }}>
                          ...
                          />
                          <button
                            onClick={() => handlePostPayment(selectedRes.id)}
                            style={{
                              padding: '0 8px',
                              height: '24px',
                              borderRadius: '4px',
                      ...
                      {selectedRes.status === 'CHECKED_IN' && (
                        <button
                          onClick={() => {
                            if (balanceDue > 0.01) {
                              alert(
                                `Outstanding Balance Due: $${balanceDue.toFixed(2)}. Please clear the due balance to check out the guest.`,
                              )
                              return
                            }
                          ...
                          style={{
                            ...
                            borderRadius: '6px',
                            background:
                              balanceDue > 0.01 ? 'rgba(255, 255, 255, 0.05)' : 'var(--primary)',
                            ...
                            fontSize: '13px',
                            boxShadow:
                              balanceDue > 0.01 ? 'none' : '0 0 10px rgba(0, 242, 254, 0.25)',
                            transition: 'all 0.2s',
                          }}
                          title={
                            balanceDue > 0.01
                              ? 'Settle all outstanding balances to check out'
                              : 'Check out guest'
                          }
                        >
                          {balanceDue > 0.01
                            ? `Check Out Locked ($${balanceDue.toFixed(2)} Due)`
                            : 'Check Out Guest (Trigger Housekeeping)'}
                        </button>
                      )}
