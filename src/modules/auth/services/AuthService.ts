...
import { UnauthorizedError, NotFoundError, ConflictError, ForbiddenError } from '@errors/HttpError'
import type {
  UserRepository,
...
export class AuthService extends BaseService {
  ...
  async register(dto: RegisterOrgOwnerDtoType, opts: TokenPersistOptions): Promise<AuthResponse> {
    return this.execute('register', async () => {
      const emailTaken = await (this.userRepo as PrismaUserRepository).emailExistsIncludingDeleted(
        dto.email,
      )
      if (emailTaken) {
      ...
      const { user } = await prisma.$transaction(async (tx: TxClient) => {
        // 1. Create user (no org linked yet)
        ...
        // 2. Create organization with user.id as ownerId
  ...
  async refreshTokens(refreshToken: string, opts: TokenPersistOptions): Promise<AuthResponse> {
