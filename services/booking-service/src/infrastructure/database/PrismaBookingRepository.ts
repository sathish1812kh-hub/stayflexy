...
function mapToGuest(r: PrismaBookingGuest): BookingGuest {
  return new BookingGuest({
    ...
    governmentIdType: r.governmentIdType as GovIdType | null,
...
export class PrismaBookingRepository implements IBookingRepository {
  ...
  async findByIdWithDetails(id: string): Promise<FullBooking | null> {
    try {
      ...
      if (!r) return null
      return {
        booking: mapToBooking(r),
        rooms: r.rooms.map(mapToRoom),
        guests: r.guests.map(mapToGuest),
      }
    } catch (err) {
      const e = fromPrismaError(err)
      if (e) throw e
      ...
      return await this.db.$transaction(async (tx) => {
            ...
            tx.bookingGuest.create({
              data: {
                ...
                governmentIdType: (g.governmentIdType ??
                  null) as PrismaBookingGuest['governmentIdType'],
        ...
        await tx.bookingAudit.create({
          data: {
            bookingId: booking.id,
            eventType: 'CREATED',
            eventDescription: 'Booking created',
            performedById: bookingData.bookedById,
          },
        })
    ...
    const where: Prisma.BookingWhereInput = {
      ...
      ...((params.guestEmail || params.guestName) && {
        guests: {
          some: {
            ...(params.guestEmail && {
              email: { contains: params.guestEmail, mode: 'insensitive' as const },
            }),
            ...(params.guestName && {
              OR: [
    ...
    const r = await this.db.booking.update({
      ...
      data: {
        ...
        ...(extra?.cancellationReason && {
          cancellationReason: extra.cancellationReason as PrismaBooking['cancellationReason'],
        }),
