...
export default function HousekeepingMobile() {
    ...
    for (const item of syncQueue) {
      try {
        await dataClient.updateRoomStatus(
          item.roomId,
          item.status,
          `Housekeeper completed checklist offline: ${item.checklist.join(', ')}`,
        )
      } catch (err) {
  ...
  const completeRoomCleaning = async () => {
    ...
    if (isOnline) {
      ...
      setRooms((prev) =>
        prev.map((r) => (r.id === selectedRoom.id ? { ...r, status: 'AVAILABLE' } : r)),
      )
    }
    ...
    >
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
          {isOnline ? (
            <>
              ...
              <span
                style={{ color: '#10b981', textTransform: 'uppercase', letterSpacing: '0.05em' }}
              >
                Sync Connected - Cloud Active
              </span>
            </>
          ) : (
            <>
              ...
              <span
                style={{ color: '#f59e0b', textTransform: 'uppercase', letterSpacing: '0.05em' }}
              >
      ...
      <header
        style={{
          ...
          background: 'linear-gradient(to bottom, rgba(16, 22, 42, 0.4), transparent)',
        }}
      >
        <div
          style={{
          ...
          {['ALL', 'HOUSEKEEPING', 'AVAILABLE', 'OCCUPIED'].map((st) => (
            <button
              ...
              style={{
                ...
                fontWeight: 600,
                border:
                  filterStatus === st ? '1px solid #a855f7' : '1px solid rgba(255,255,255,0.05)',
                background:
                  filterStatus === st ? 'rgba(168, 85, 247, 0.08)' : 'rgba(255,255,255,0.02)',
        ...
        >
          ...
          >
            <div>
              ...
              <h2 style={{ fontSize: '18px', fontWeight: 700, color: '#fff', margin: '2px 0 0' }}>
                Clean Room {selectedRoom.roomNumber}
              </h2>
            </div>
            <button
          ...
          >
            ...
            {[
              {
                id: 'linen',
                label: 'Linen & Bedding Refresh',
                desc: 'Swap all towels, sheets, pillowcases with sanitised items.',
              },
              {
                id: 'dusting',
                label: 'Dusting & Surface Sanitation',
                desc: 'Disinfect desks, TV remotes, light switches and sideboards.',
              },
              {
                id: 'bathroom',
                label: 'Bathroom Disinfection & Polish',
                desc: 'Sanitise toilet, shower stall, mirrors, and replenish soaps.',
              },
              {
                id: 'minibar',
                label: 'Minibar & Amenities Restock',
                desc: 'Verify minibar counters, refill custom snacks, waters, coffees.',
              },
            ].map((item) => (
              <button
                key={item.id}
                ...
                <div style={{ flex: 1 }}>
                  <div
                    style={{
                      fontSize: '13px',
                      fontWeight: 600,
                      color: checkedItems[item.id] ? '#fff' : '#cbd5e1',
                    }}
                  >
                    {item.label}
                  </div>
