...
export class BookingController {
  ...
  private getAuth(req: Request) {
    ...
    if (!userId || !orgId) throw new UnauthorizedError('Missing user or organization context')
  ...
  create = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
    try {
      ...
      const dto = validate(createBookingSchema as any, req.body) as CreateBookingDto
  ...
  search = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
    try {
      const { orgId, correlationId } = this.getAuth(req)
      const dto = validate(
        searchBookingSchema as any,
        req.query as Record<string, string>,
      ) as SearchBookingDto
      const result = await this.searchBookingsUC.execute(dto, orgId)
      res.json({
        success: true,
        data: result.data.map(serializeFullBooking),
        meta: result.meta,
        correlationId,
      })
    } catch (err) {
