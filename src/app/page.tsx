...
export default function DashboardPage() {
  ...
  useEffect(() => {
    ...
    loadData()
    // Poll the backend so room / room-type changes made in ANOTHER browser
    // window appear here within a few seconds — no manual reload needed.
    // Visible-tab only, to avoid needless background load.
  ...
  useEffect(() => {
    async function loadReservations() {
      // Prefer the BACKEND so both browser windows show the same bookings.
      // Previously this read per-browser localStorage, which is why dashboard
      // values diverged between windows. Falls through to local demo/last-known
      // data only when the backend has no bookings or is unreachable.
      try {
        const activeHotelId =
          typeof window !== 'undefined'
            ? localStorage.getItem('sf_selected_hotel') || 'h1-resort-goa'
            : 'h1-resort-goa'
        const [remoteBookings, physicalRooms] = await Promise.all([
          dataClient.getBookings(activeHotelId),
          dataClient.getRooms(),
        ])
        if (remoteBookings && remoteBookings.length > 0) {
          const mapped = remoteBookings.map((b: any) => {
            const g = b.guests?.find((x: any) => x.isPrimary) || b.guests?.[0]
            const room = b.rooms?.[0]
            const roomId = room?.roomId || ''
            const physicalRoom = physicalRooms.find((r) => r.id === roomId)
            return {
              id: b.id,
              guestName: g ? `${g.firstName} ${g.lastName}` : 'Guest Stayflexi',
              roomNumber: physicalRoom ? physicalRoom.roomNumber : '',
              roomId,
              checkIn: room?.checkInDate ? room.checkInDate.split('T')[0] || '' : '',
              checkOut: room?.checkOutDate ? room.checkOutDate.split('T')[0] || '' : '',
              status: b.status,
              amount:
                b.rooms?.reduce((acc: number, r: any) => acc + (r.totalRoomAmount || 0), 0) || 0,
    ...
    loadReservations()
    // Poll so bookings made in another browser window appear here within ~5s.
    const resPoll = setInterval(() => {
      if (typeof document === 'undefined' || document.visibilityState === 'visible')
        loadReservations()
    }, 5000)
    return () => clearInterval(resPoll)
  }, [])
  ...
  useEffect(() => {
    async function loadAnalytics() {
      ...
      try {
        ...
        const startDateStr = new Date(Date.now() - daysToSubtract * 24 * 60 * 60 * 1000)
          .toISOString()
          .substring(0, 10)
        const metrics = await dataClient.getRevenueMetrics(
          'h1-resort-goa',
          startDateStr,
          endDateStr,
        )
  ...
  const filterReservationsByDate = (
    list: Reservation[],
    range: 'daily' | 'weekly' | 'monthly' | '10days',
  ) => {
    ...
    return list.filter((res) => {
      ...
      if (range === 'daily') {
        return (
          res.checkIn === SIMULATED_TODAY ||
          res.checkOut === SIMULATED_TODAY ||
  ...
  const handleCheckIn = async (bookingId: string, roomId: string) => {
    ...
    await dataClient.updateRoomStatus(
      roomId,
      'OCCUPIED',
      `Checked in guest via PMS Operations Console`,
    )
  ...
  const handleCheckOut = async (bookingId: string, roomId: string) => {
    ...
    if (balanceDue > 0.01) {
      alert(
        `Guest has an outstanding balance of $${balanceDue.toFixed(2)}. Please settle dues first.`,
      )
      return
    }
    ...
    if (roomId) {
      await dataClient.updateRoomStatus(
        roomId,
        'HOUSEKEEPING',
        `Checked out guest via PMS Operations Console`,
      )
      const updatedRooms = await dataClient.getRooms()
      setRooms([...updatedRooms])
    }
  ...
  const handleExtendStay = (bookingId: string) => {
    ...
    saveAndReloadReservations(updated)
    alert(
      `Stay extended for ${booking.guestName} to ${newCheckOutStr}. Charge of $150.0 post to folio.`,
    )
  }
  ...
  const handleConfirmHold = (bookingId: string) => {
    ...
    saveAndReloadReservations(updated)
    alert(
      `Tentative hold confirmed for reservation #${bookingId.toUpperCase()}. Moved to expected arrivals list.`,
    )
  }
  ...
  const handleToggleVerification = (
    bookingId: string,
    field: 'ocrStatus' | 'signatureCollected' | 'selfieVerified',
  ) => {
  ...
  const handleChannelSync = async (providerId: string, providerName: string) => {
    ...
    try {
      ...
      alert(
        `Channel synced successfully! Rates, inventories, and bookings matched to ${providerName}.`,
      )
    } catch (err) {
      console.error(err)
  ...
  const handleSendMagicLink = (guestName: string, email?: string) => {
    alert(
      `Pre-check-in link dispatched to ${email || 'guest@example.com'} for guest ${guestName}. Guest can upload ID documents and sign contactless agreements via mobile.`,
    )
  }

  // Submit booking wizard
  const handleCreateNewBookingSubmit = () => {
    const { firstName, lastName, email, phone, checkIn, checkOut, roomTypeId, amount, status } =
      newBookingForm
    ...
    alert(
      `Successfully registered reservation #${newId.toUpperCase()} for ${firstName} ${lastName}!`,
    )
  }
    ...
    for (let i = 0; i < resList.length; i++) {
      for (let j = i + 1; j < resList.length; j++) {
        ...
        if (r1.checkIn < r2.checkOut && r2.checkIn < r1.checkOut) {
          const customRes = !r1.id?.endsWith('-default')
            ? r1
            : !r2.id?.endsWith('-default')
              ? r2
              : null
          ...
          if (!alertsList.some((a) => a.id === alertId)) {
            alertsList.push({
              ...
              action: () => {
                ...
                saveAndReloadReservations(updated)
                alert(
                  `Double booking resolved: Removed duplicate booking for ${targetRes.guestName}.`,
                )
              },
            })
          }
  ...
  const arrivalsCount = dateFilteredRes.filter(
    ...
    >
      ...
      <section className="metrics-grid">
        ...
        <div className="glass-card">
          ...
          <div className="metric-value">
            {availableRooms}{' '}
            <span style={{ fontSize: '14px', color: 'var(--text-muted)' }}>/ {totalRooms}</span>
          </div>
        ...
        <div className="glass-card">
          ...
          <div className="metric-value">
            {activeHotels}{' '}
            <span style={{ fontSize: '14px', color: 'var(--text-muted)' }}>/ {hotels.length}</span>
          </div>
          <div className="metric-trend" style={{ color: '#a855f7' }}>
            <span>
              {hotels.filter((h) => h.status === 'UNDER_RENOVATION').length} under renovation
            </span>
          </div>
        </div>
      ...
      <div
        ...
        style={{
          ...
          background: 'linear-gradient(to bottom, var(--bg-card), rgba(0, 242, 254, 0.01))',
          marginTop: '24px',
          marginBottom: '24px',
        }}
      >
        ...
        >
          <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
            ...
            <h3 style={{ fontSize: '16px', fontWeight: 700 }}>
              Supergraph Financial Analytics Console
            </h3>
          </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr', gap: '24px' }}>
              ...
              >
                ...
                <div
                  style={{
                    fontSize: '22px',
                    fontWeight: 800,
                    color: activeOpsTab === 'arrivals' ? '#f59e0b' : '#fff',
                    marginTop: '6px',
                  }}
                >
                  {arrivalsCount}
              ...
              >
                ...
                <div
                  style={{
                    fontSize: '22px',
                    fontWeight: 800,
                    color: activeOpsTab === 'inhouse' ? '#10b981' : '#fff',
                    marginTop: '6px',
                  }}
                >
                  {inHouseCount}
              ...
              >
                ...
                <div
                  style={{
                    fontSize: '22px',
                    fontWeight: 800,
                    color: activeOpsTab === 'departures' ? '#ef4444' : '#fff',
                    marginTop: '6px',
                  }}
                >
                  {departuresCount}
              ...
              >
                ...
                <div
                  style={{
                    fontSize: '22px',
                    fontWeight: 800,
                    color: activeOpsTab === 'magiclist' ? '#a855f7' : '#fff',
                    marginTop: '6px',
                  }}
                >
                  {magicListCount}
              ...
              >
                ...
                <div
                  style={{
                    fontSize: '22px',
                    fontWeight: 800,
                    color: activeOpsTab === 'onhold' ? '#f59e0b' : '#fff',
                    marginTop: '6px',
                  }}
                >
                  {onHoldCount}
              ...
              >
                ...
                <div
                  style={{
                    fontSize: '22px',
                    fontWeight: 800,
                    color: activeOpsTab === 'noshow' ? '#ef4444' : '#fff',
                    marginTop: '6px',
                  }}
                >
                  {noShowCount}
              ...
              >
                ...
                <div
                  style={{
                    fontSize: '22px',
                    fontWeight: 800,
                    color: activeOpsTab === 'cancelled' ? '#6b7280' : '#fff',
                    marginTop: '6px',
                  }}
                >
                  {cancelledCount}
                        ...
                        <div style={{ display: 'flex', gap: '8px' }}>
                          ...
                          {activeOpsTab === 'cancelled' && (
                            <button
                              onClick={() => {
                                const updated = reservations.map((r) =>
                                  r.id === res.id ? { ...r, status: 'PENDING' as const } : r,
                                )
                                saveAndReloadReservations(updated)
                              ...
                              <div style={{ display: 'flex', gap: '8px', alignItems: 'center' }}>
                                ...
                                >
                                  <input
                                    ...
                                    onChange={() =>
                                      handleToggleVerification(res.id, 'signatureCollected')
                                    }
                                  />
                                  <span
                                    style={{
                                      color: res.signatureCollected
                                        ? '#10b981'
                                        : 'var(--text-muted)',
                                    }}
                                  >
                                ...
                                >
                                  <input
                                    ...
                                    onChange={() =>
                                      handleToggleVerification(res.id, 'selfieVerified')
                                    }
                                  />
                                  <span
                                    style={{
                                      color: res.selfieVerified ? '#10b981' : 'var(--text-muted)',
                                    }}
                                  >
                                    Selfie
                                  </span>
          ...
          <div className="glass-card" style={{ padding: '24px' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: '16px' }}>
              ...
              >
                ...
                <p style={{ fontSize: '11px', color: 'var(--text-muted)', lineHeight: '1.4' }}>
                  Rates, room mappings, and calendar modifications synced in real time. Webhook
                  active.
                </p>
                <div
                  style={{
            ...
            <div style={{ marginTop: '20px' }}>
              ...
              >
                ...
                >
                  Dynamic OTA Sync Jobs History
                </h4>
                <span style={{ fontSize: '10px', color: '#10b981', fontWeight: 600 }}>
                  ● CHANNEL GATEWAY ACTIVE
                </span>
              </div>
              <div
                style={{
                ...
                <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '11px' }}>
                  ...
                  <tbody>
                    {syncJobs.slice(0, 5).map((job, idx) => {
                      const providerName =
                        job.providerId === 'prov-booking'
                          ? 'Booking.com'
                          : job.providerId === 'prov-agoda'
                            ? 'Agoda'
                            : job.providerId === 'prov-expedia'
                              ? 'Expedia'
                              : job.providerId === 'prov-airbnb'
                                ? 'Airbnb'
                                : 'OTA Channel'
                      return (
                        <tr
          ...
          <div className="glass-card">
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '18px' }}>
              {hotels.map((hotel) => {
                ...
                const pct =
                  hotelRooms.length > 0 ? Math.round((hotelOccupied / hotelRooms.length) * 100) : 0

                return (
                  <div
                    key={hotel.id}
                    style={{ borderBottom: '1px solid var(--border-card)', paddingBottom: '14px' }}
                  >
                    <div
                      style={{
                        display: 'flex',
                    ...
                    >
                      ...
                      <div style={{ textAlign: 'right' }}>
                        <span
                          className={`status-badge ${hotel.status === 'ACTIVE' ? 'available' : 'blocked'}`}
                        >
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
          ...
          <div
            ...
            style={{
              padding: '20px',
              background: 'linear-gradient(to bottom, var(--bg-card), rgba(0, 242, 254, 0.02))',
              textAlign: 'center',
            }}
          >
            <div
            ...
            <div style={{ fontSize: '11px', color: 'var(--text-muted)', lineHeight: '1.4' }}>
              Have guests scan QR at the front desk to open remote check-in details sheet on their
          ...
          <div
            ...
            style={{
              ...
              background: 'linear-gradient(to bottom, var(--bg-card), rgba(168, 85, 247, 0.02))',
            }}
          >
            <div
            ...
            {aiInsightSuccess && (
              ...
              >
                ...
                <button
                  onClick={() => setAiInsightSuccess(null)}
                  style={{
                    background: 'none',
                    border: 'none',
                    color: '#fff',
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
              ...
              >
                <div
                  style={{ fontSize: '12px', fontWeight: 600, color: '#fff', marginBottom: '4px' }}
                >
                  Calendar Auto-Compactor
                </div>
                <p
                  style={{
                    fontSize: '11px',
                ...
                >
                  Optimize layout configurations to close 3 single-day gaps in Deluxe Category.
                  Re-arrange non-checked-in slots.
                </p>
                <button
                  onClick={async () => {
                    ...
                    setAiInsightSuccess(
                      'AI Tetris successfully reshuffled allocations! Opened Room 101 for a continuous 4-day reservation slot.',
                    )
                  }}
              ...
              >
                ...
                >
                  2 suites are empty today between 10:00 AM and 3:00 PM. AI recommends enabling a
                  3-hour micro-stay rate of $45.00.
                </p>
                <button
                  onClick={() => {
                    setAiInsightSuccess(
                      'Micro-stay rate slots pushed successfully to Booking.com, Agoda and Airbnb!',
                    )
                  }}
                  style={{
                    background: 'rgba(255,255,255,0.03)',
              ...
              >
                ...
                >
                  Goa regional search indexes show 40% uptick in deluxe room inquiries. AI suggests
                  raising weekend rate to $180.
                </p>
                <button
                  onClick={() => {
                    setAiInsightSuccess(
                      'Parity rate optimization applied! New rates synced to Booking.com and direct engines.',
                    )
                  }}
                  style={{
                    background: 'rgba(255,255,255,0.03)',
            ...
            >
              ...
              {dirtyRooms.length === 0 ? (
                ...
                >
                  {dirtyRooms.map((rm) => {
                    const rtName =
                      roomTypes.find((t) => t.id === rm.roomTypeId)?.name || rm.roomTypeId
                    return (
                      <div
                        key={rm.id}
                        style={{
          ...
          <div
            className="glass-card"
            style={{
              background: 'linear-gradient(to bottom, var(--bg-card), rgba(79, 172, 254, 0.05))',
            }}
          >
            <h3
              style={{
            ...
            >
              The dynamic Apollo Federated Supergraph composed successfully, exposing combined
              entities under a type-safe schema layer.
            </p>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px' }}>
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setNewBookingForm({ ...newBookingForm, firstName: e.target.value })
                    }
                  />
                </div>
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setNewBookingForm({ ...newBookingForm, lastName: e.target.value })
                    }
                  />
                </div>
              </div>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px' }}>
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setNewBookingForm({ ...newBookingForm, email: e.target.value })
                    }
                  />
                </div>
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setNewBookingForm({ ...newBookingForm, phone: e.target.value })
                    }
                  />
                </div>
              </div>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px' }}>
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setNewBookingForm({ ...newBookingForm, checkIn: e.target.value })
                    }
                  />
                </div>
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setNewBookingForm({ ...newBookingForm, checkOut: e.target.value })
                    }
                  />
                </div>
              </div>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1.5fr 1fr', gap: '12px' }}>
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      setNewBookingForm({ ...newBookingForm, roomTypeId: e.target.value })
                    }
                    style={{ cursor: 'pointer' }}
                  >
                    {roomTypes.map((rt) => (
...
const actionBtnStyle = (
  colorType: 'green' | 'red' | 'gray' | 'purple' | 'disabled',
): React.CSSProperties => {
