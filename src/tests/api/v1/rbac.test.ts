...
test.describe('Role Administration API (Phase 2)', () => {
  ...
  test('GET /api/v1/users returns organization staff directory', async ({ request }) => {
    const admin = await registerAndLogin(request, 'user-directory')
    ...
    expect(body.data.some((u) => u.id === admin.userId)).toBe(true)
  })

  test('Assigning custom role safely falls back without Prisma enum error', async ({ request }) => {
    const admin = await registerAndLogin(request, 'custom-assign')

    // 1. Create custom role
    const createRoleRes = await request.post('/api/v1/roles', {
      headers: { Authorization: `Bearer ${admin.accessToken}` },
      data: {
        name: `Concierge Specialist ${Date.now()}`,
        description: 'Specialist custom role',
        permissionIds: [],
      },
    })
    expect(createRoleRes.status()).toBe(201)
    const customRole = ((await createRoleRes.json()) as { data: { id: string } }).data

    // 2. Assign custom role to user
    const assignRes = await request.post(`/api/v1/users/${admin.userId}/roles`, {
      headers: { Authorization: `Bearer ${admin.accessToken}` },
      data: { roleId: customRole.id },
    })
    expect(assignRes.status()).toBe(201)

    // 3. User me endpoint remains healthy
