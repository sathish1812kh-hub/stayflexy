  ...
  async findByOrganization(params: PaymentSearchParams): Promise<PaginatedResult<Payment>> {
    ...
    const where: Prisma.PaymentWhereInput = {
      ...
      ...(params.paymentStatus && {
        paymentStatus: params.paymentStatus as PrismaPayment['paymentStatus'],
      }),
      ...(params.paymentMethod && {
        paymentMethod: params.paymentMethod as PrismaPayment['paymentMethod'],
      }),
      ...
      const r = await this.db.$transaction(async (tx) => {
        ...
        await tx.paymentAudit.create({
          data: {
            ...
            eventType: 'CREATED',
            eventDescription: 'Payment initiated',
            performedById: data.processedById,
          },
        })
        return payment
      })
