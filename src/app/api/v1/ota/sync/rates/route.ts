...
export const POST = withPermission("ota", "sync", async (req: NextRequest, { user }) => {
  try {
    ...
    const dto = v.validateTriggerSync({ ...(body as object), syncType: "RATE_PUSH" });
    const { syncJobService } = await import("@modules/synchronization/container");
    const job = await syncJobService.createSyncJob(
      { hotelId: dto.hotelId, providerId: dto.providerId, syncType: "RATE_PUSH", payload: dto.payload, maxRetries: dto.maxRetries },
      user.id,
      user.organizationId ?? ""
    );
