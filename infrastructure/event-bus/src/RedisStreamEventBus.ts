...
export class RedisStreamEventBus implements IEventBus {
  ...
  async publish(event: Omit<DomainEventEnvelope, 'eventId' | 'timestamp'>): Promise<string> {
  ...
  private async pollOnce(eventType: string, state: SubscriptionState): Promise<void> {
    const key = streamKey(eventType)
