...
// POST /api/v1/compliance/delete-request  { requestType, subjectUserId, notes }
export const POST = withPermission("compliance", "create", async (req: NextRequest, { user }) => {
  try {
    ...
    const request = await complianceService.createDeletionRequest(dto, user.id, user.organizationId ?? "");
